...
CMD_RUN="docker run --name $PORTAINER_CONTAINER_NAME --network $CONTAINER_NETWORK -d --restart=always -v /var/run/docker.sock:/var/run/docker.sock -v $HOME/temp/monitoring/portainer:/data $PORTAINER_CONTAINER_IMAGE_NAME:$PORTAINER_CONTAINER_IMAGE_VERSION"
#CMD_RUN="docker run --name $PORTAINER_CONTAINER_NAME --network $CONTAINER_NETWORK -d -p 8001:8000 -p 9001:9000 --restart=always -v /var/run/docker.sock:/var/run/docker.sock -v $HOME/temp/monitoring/portainer:/data $PORTAINER_CONTAINER_IMAGE_NAME:$PORTAINER_CONTAINER_IMAGE_VERSION"
