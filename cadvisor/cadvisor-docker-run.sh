...
#CMD_RUN="docker run --name $CADVISOR_CONTAINER_NAME --network $MONITORING_NETWORK -d -p 8081:8080 -v /:/rootfs:ro -v /var/run:/var/run:rw -v /sys:/sys:ro -v /var/lib/docker/:/var/lib/docker:ro $CADVISOR_CONTAINER_IMAGE_NAME:$CADVISOR_CONTAINER_IMAGE_VERSION -url_base_prefix=/cadvisor"
CMD_RUN="docker run --name $CADVISOR_CONTAINER_NAME --network $MONITORING_NETWORK -d -v /:/rootfs:ro -v /var/run:/var/run:rw -v /sys:/sys:ro -v /var/lib/docker/:/var/lib/docker:ro $CADVISOR_CONTAINER_IMAGE_NAME:$CADVISOR_CONTAINER_IMAGE_VERSION -url_base_prefix=/cadvisor"
