...
#CMD_RUN="docker run --name $GRAFANA_CONTAINER_NAME --network $CONTAINER_NETWORK -d -p 8085:3000 -v $HOME/dev/windfire-proxy/grafana/conf:/usr/share/grafana/conf -v $HOME/temp/grafana:/var/lib/grafana $GRAFANA_CONTAINER_IMAGE_NAME:$GRAFANA_CONTAINER_IMAGE_VERSION"
CMD_RUN="docker run --name $GRAFANA_CONTAINER_NAME --network $CONTAINER_NETWORK -d -v $HOME/dev/windfire-proxy/grafana/conf:/usr/share/grafana/conf -v $HOME/temp/grafana:/var/lib/grafana $GRAFANA_CONTAINER_IMAGE_NAME:$GRAFANA_CONTAINER_IMAGE_VERSION"
